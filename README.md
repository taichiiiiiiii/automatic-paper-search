...
**主要な出力:** GitHub Pages 上のインタラクティブ家系図ビュー（`docs/<conference>/lineage.html`、`.github/workflows/pages.yml` でデプロイ）。サイト上のフォームから新規テーマ投稿 → Vercel Function (`api/themes.js`) → Issue 作成 → ラベル trigger workflow が `theme-on-demand.yml` を自動 dispatch して生成。
...
- **オンデマンドテーマ生成**（フォーム経由、`theme-on-demand.yml`）— サイト上のフォームから新テーマを 1 件だけ生成、Vercel Function `api/themes` 経由で Issue 作成 → ラベル trigger workflow が自動 dispatch
