...
**主要な出力:** GitHub Pages 上のインタラクティブ家系図ビュー（`docs/<conference>/lineage.html`、`.github/workflows/pages.yml` でデプロイ）。静的のみ。新規テーマは GitHub Issue (`theme-request` テンプレ) → 運用者の手動 dispatch で生成。
...
- **オンデマンドテーマ生成**（手動 dispatch、`theme-on-demand.yml`）— GitHub Issue (`theme-request` テンプレ) で受付、運用者が `gh workflow run` で 1 件だけ生成
