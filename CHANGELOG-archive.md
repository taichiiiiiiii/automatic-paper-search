# Changelog Archive — PaperPilot

`CHANGELOG.md` の `[Unreleased]` から、**完了済みの履歴サブセクション 20 本**
（#209 系 S2-free 移行 / CF Worker 復活 #233–#238 / unarXive Phase J #222 /
lineage 品質改善 Tier 1・audit / #285 prep ほか）をそのまま切り出した保管庫。
**本文は一字一句そのまま**で、要約も削除もしていない。現行の変更履歴は
[`CHANGELOG.md`](CHANGELOG.md) を見ること。

- 切り出し日: 2026-08-18
- 切り出し範囲: 切り出し前 `CHANGELOG.md` の 93–693 行目
- 無損失の検証方法:
  ```bash
  { sed -n '1,92p' <(git show <切り出し前>:CHANGELOG.md); \
    sed -n '/^### Added — Lineage edge provenance field/,$p' CHANGELOG-archive.md; \
    sed -n '694,792p' <(git show <切り出し前>:CHANGELOG.md); } | md5sum
  # → 切り出し前 CHANGELOG.md の md5 と一致する
  ```
