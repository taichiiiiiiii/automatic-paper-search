...
14. **テーマ家系図 (`docs/themes/<slug>/lineage.json`) は `build_theme_lineage.py` が唯一の生成元。手編集禁止**
    ...
    - **LLM rationale 完全化**: `theme-on-demand.yml` は `--llm-strict=all` を有効化。全 influential edge を Groq (Llama 3.3 70B) で paper-specific 分類。worst case ~40 LLM call/theme、Groq free tier 30 RPM で +90s。Groq 429 throttle 時は heuristic フォールバック (silent fallback ではなく log warning + classify_attempted/succeeded 件数集計あり、#45)。コスト懸念があれば `--llm-strict=ambiguous` (S2 intent 不明 edge のみ、~5-15 calls/theme、+30s) に下げる。
