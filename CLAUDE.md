...
14. **テーマ家系図 (`docs/themes/<slug>/lineage.json`) は `build_theme_lineage.py` が唯一の生成元。手編集禁止**
    ...
    - **品質改善ノイズ防止**: 2 レイヤで off-topic 論文を除外する:
        ...
        2. **Foundational ref フィルタ (`_filter_off_topic_parents`)**: BFS で取得した parent/child 候補のうち、`citationCount > 3 × max(seed citations)` かつ S2 intent に "methodology" を含まないものを除外。これで「GNN テーマの seed が ResNet (228k cites) を引用しているが、その ResNet は GNN 系譜の一部ではない」ケースが排除される。"methodology" 意図がある場合はそのまま採用（その citing paper の手法を本当に支えている foundational ref のため）。
    - **LLM rationale 強化**: `theme-on-demand.yml` は `--llm-strict=ambiguous` を有効化。S2 intent から heuristic で relation が決まらない edge (`_is_ambiguous(intent_record) == True`) だけ Groq (Llama 3.3 70B) で paper-specific な rationale を生成。テーマあたり 5-15 LLM call で workflow 実行時間 +30s 程度。Groq 429 throttle 時は heuristic フォールバック (silent fallback ではなく log warning + 件数集計あり、#45)。完全 LLM 化が必要な場合は `--llm-strict=all` (~30-50 calls/theme、+90s)。
