...
14. **テーマ家系図 (`docs/themes/<slug>/lineage.json`) は `build_theme_lineage.py` が唯一の生成元。手編集禁止**
    ...
    - **並列 dispatch の push 競合対策**（#121）: Worker は per-IP 5/h + global 100/day で並列 dispatch を許す設計のため、複数の `theme-on-demand` run が同時刻に `develop` へ push すると 1 本以外が `! [rejected] develop -> develop (fetch first)` で discard されていた。対策は二段構え:
        1. `concurrency: { group: themes-push-develop, cancel-in-progress: false }` を `theme-on-demand.yml` と `regen-themes.yml` に設定。GitHub Actions の concurrency は **ジョブ全体** を serialize（push step 単独ではない）するため、両 workflow が同 group にいる間は1度に1ランしか走らない。`cancel-in-progress: false` 必須（LLM/S2 で 2-3 分かけた生成結果を捨てない）。**トレードオフ**: 週次 regen 実行中（最大 60 分）はオンデマンドが queue で待機する。発火曜日 (Sun 09:00 JST) は研究者活動時間に重なるが、retry の単純さと race 完全排除を優先。
        2. push step は `.github/scripts/commit-and-push.sh` 経由で 5 回 retry（`git fetch + git rebase --autostash + git push`、各失敗で `git rebase --abort` リカバリ、ジッタ付き sleep）。`COMMIT_PUSH_NO_SLEEP=1` で sleep 無効化、`COMMIT_PUSH_MAX_ATTEMPTS=N` で回数上書き（テスト用）。
        - shell ロジックは `paperpilot/tests/test_commit_and_push_sh.py` が subprocess 経由でカバー（並列 push race + injection 安全 + max-retry exhaustion）。
