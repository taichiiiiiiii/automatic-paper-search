...
> - 🔴 §11 が示す `.github/workflows/collect.yml` は**存在しません**。日次 cron による自動収集は行われていません。
> - 🔴 「週次で E2E を本番 API に実行」も成立しません（`collect-weekly.yml` は #245 で cron を外し手動専用）。
>   2026-08-20 現在、定期実行される workflow は **`lighthouse.yml` ただ 1 本**です。
