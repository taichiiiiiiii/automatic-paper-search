...
> ## 📖 まずこちらを読む: [38-unified-current-design.md](38-unified-current-design.md)（2026-09-20）
>
> 08〜37 番の現行設計・実装状況を1本の文書に要約した参照用ドキュメントです。個別の受入テスト条件・
> JSON Schemaの一字一句・詳細な実装手順が必要な場合は、本文中に示す元文書番号（例: `[11]`, `[18]`）を辿ってください。
> 13〜15 番は他ファイル（AGENTS.md / `paperpilot/scripts/README.md` / テスト）から直接パス参照されるため、
> 統合文書とは別に単独ファイルとして維持しています。
