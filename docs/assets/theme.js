...
const { escapeHtml, formatStars, dataRoot } = window.PP;
...
const els = {
  ...
  // On-demand theme submission form + progress panel refs live in
  // theme-request.js (auto-binds on load; works on /themes/ and /lineage/).
...
async function loadManifest() {
  try {
    ...
    // dataRoot() prefixes the page-relative path with the data-root
    // meta (set by /lineage/index.html = "../"). /themes/index.html is
    // now a redirect stub and no longer loads theme.js; dataRoot() = ""
    // only applies to direct DOM mounts in tests.
    const res = await fetch(dataRoot() + "themes/themes-manifest.json", { cache: "no-cache" });
    if (!res.ok) return [];
...
async function loadQuality() {
  ...
  // renders.
  try {
    const res = await fetch(dataRoot() + "themes/_quality.json", { cache: "no-cache" });
    if (!res.ok) return {};
    const data = await res.json();
...
async function loadThemeLineage(slug) {
  ...
  // With dataRoot() prefixed, /lineage/?theme=<slug> (dataRoot() =
  // "../") resolves to /themes/<slug>/lineage.json. The legacy
  // /themes/ page is a redirect stub and no longer loads this file.
  try {
    ...
    const res = await fetch(`${dataRoot()}themes/${slug}/lineage.json`, { cache: "no-cache" });
...
async function init() {
  // Activation gate for the unified /lineage/ viewer. That page hosts
  // three view modes (selector / theme / tree) in one HTML, with
  // lineage-shell.js routing by URL param. The theme viewer's mount
  // carries `data-viewer="theme"`; when that marker is present AND no
  // ?theme= was supplied, bail out so the shell can show the
  // selector or dispatch to the tree controller. /themes/index.html
  // has no marker, so the gate is transparent there and the viewer
  // keeps its legacy "always run" behaviour.
  const mount = document.querySelector('[data-viewer="theme"]');
  if (mount) {
    const hasTheme = new URLSearchParams(window.location.search).get("theme");
    if (!hasTheme) return;
  }
  // Defence-in-depth: when the canvas/svg are missing entirely the
  // page isn't hosting the viewer at all (e.g. a stub page that
  // only loads the shell).
  if (!document.getElementById("lineage-svg") || !document.getElementById("canvas")) {
    return;
  }

  // Theme-request form binding moved to theme-request.js (auto-binds
  // on load). /lineage/ loads the same script so the compact form
  // under the selector shares the submission flow.
  bindHeroToggle();
