...
function renderThemeGallery() {
  ...
  for (const entry of state.manifest) {
    ...
    // Quality badge: 🟢 specific (low template ratio), 🟡 mixed,
    // 🔴 generic (mostly templated edges). Skip the badge entirely when
    // _quality.json hasn't loaded for this slug — better than confusing
    // the user with a stale or guessed signal.
    const q = state.quality?.[entry.slug];
    if (q && typeof q.template_ratio === "number") {
      const ratio = q.template_ratio;
      const badge = document.createElement("div");
      badge.className = "theme-gallery__quality";
      let icon, label;
      if (ratio < 0.30) { icon = "🟢"; label = "specific"; }
      else if (ratio < 0.70) { icon = "🟡"; label = "mixed"; }
      else { icon = "🔴"; label = "generic"; }
      badge.textContent = `${icon} ${label}`;
      // Tooltip with the raw numbers so power users can drill down.
      badge.title = `template_ratio=${Math.round(ratio * 100)}% (${q.template_count}/${q.edge_count} edges) · off_topic_focus=${q.off_topic_focus}`;
