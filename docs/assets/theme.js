...
//   - ../lineage-quality-v1.json (shared fail-closed quality read model;
//     a theme renders only when its row is ready+passed and the artifact
//     bytes match the audited input_sha256)
//   - themes/<slug>/lineage.json  (per-theme tree, generated by build_theme_lineage.py;
//     fetched only through the quality gate, parsed as strict lineage-artifact-v1)
//
...
//   - The artifact fetch URL is assembled ONLY from the quality row's
//     validated path (LineageCore.parseQualityManifest enforces the strict
//     themes/<slug>/lineage.json shape) — the raw URL slug never reaches a
//     fetch URL.
...
// Shared fail-closed lineage reader (lineage-core.js is loaded before this
// file in themes/index.html). All quality gating, byte-hash verification and
// strict artifact parsing go through it — theme.js never hand-parses.
const LineageCore = window.PaperPilotLineageCore;
const REQUIRED_LINEAGE_CORE_METHODS = [
  "parseQualityManifest", "resolveQualityCollection", "qualityRowIsEligible",
  "qualityRowIsPublishable", "fetchJsonWithSha256", "parseArtifact", "resolveFocus",
];

function lineageCoreIsAvailable() {
  return LineageCore !== null && typeof LineageCore === "object"
    && REQUIRED_LINEAGE_CORE_METHODS.every((name) => typeof LineageCore[name] === "function");
}
...
const state = {
  ...
  quality: {},
  // Strict-parsed ../lineage-quality-v1.json (shared read model with the
  // conference/deep viewers). null = missing/malformed → every theme stays
  // fail-closed. This gate alone authorises artifact fetch+render; the
  // _quality.json rollup above is badge telemetry only.
  lineageQuality: null,
  currentSlug: null,
...
const els = {
  ...
  readyUi: document.getElementById("lineage-ready-ui"),
  auditStatus: document.getElementById("lineage-audit-status"),
};
...
async function loadLineageQuality() {
  // Shared fail-closed quality read model (docs/lineage-quality-v1.json, one
  // directory up). Strict parse: any schema drift leaves every row
  // ineligible, so no artifact is ever fetched.
  if (!lineageCoreIsAvailable()) return null;
  try {
    const loaded = await LineageCore.fetchJsonWithSha256(
      "../lineage-quality-v1.json", { cache: "no-cache" },
    );
    return loaded ? LineageCore.parseQualityManifest(loaded.data) : null;
  } catch {
    return null;
  }
}

function eligibleThemeManifest(manifest, quality) {
  if (!lineageCoreIsAvailable() || !Array.isArray(manifest) || !quality) return [];
  return manifest.filter((entry) => {
    if (typeof entry?.slug !== "string" || !SLUG_RE.test(entry.slug)) return false;
    const row = LineageCore.resolveQualityCollection(quality, {
      kind: "theme", slug: entry.slug,
    });
...
function showReadyUi() {
  if (els.auditStatus) els.auditStatus.hidden = true;
  if (els.readyUi) els.readyUi.hidden = false;
  if (els.heroToggle) els.heroToggle.hidden = false;
  if (els.heroNewTheme) els.heroNewTheme.hidden = false;
  if (els.reqForm) els.reqForm.hidden = false;
  const title = document.getElementById("lineage-page-title");
  if (title) title.textContent = "Theme Lineage — テーマで時系列家系図";
}

async function loadThemeArtifact(slug) {
  // P2T quality gate: fetch and render only when this theme's quality row is
  // ready+passed, the fetched bytes hash to the audited input_sha256, and the
  // payload parses as a strict lineage-artifact-v1 with kind="theme". The
  // fetch URL is assembled from the validated row.path — the raw slug is
  // never spliced into a URL. Missing/failed/malformed quality, hash
  // mismatch, or parse failure all collapse to null (no fetch, no render).
  if (!lineageCoreIsAvailable()) return null;
  const qualityRow = LineageCore.resolveQualityCollection(state.lineageQuality, {
    kind: "theme", slug,
  });
  if (!LineageCore.qualityRowIsEligible(qualityRow)) return null;
  // cache: "no-cache" — per-theme lineage.json can be regenerated by the
  // weekly cron or a re-dispatch, so revalidate via ETag rather than serving
  // from a 5 min max-age. 304 is the common case.
  const loaded = await LineageCore.fetchJsonWithSha256(
    `../${qualityRow.path}`, { cache: "no-cache" },
    { expectedSha256: qualityRow.input_sha256 },
  );
  if (!loaded || !LineageCore.qualityRowIsPublishable(qualityRow, {
    artifactSha256: loaded.sha256,
  })) return null;
  return LineageCore.parseArtifact(loaded.data, { kind: "theme" });
}
...
//   node   — permalink focus, resolved in init() via LineageCore.resolveFocus
...
const REQUEST_ID_RE = /^theme-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}(?![\s\S])/;
...
function statusUrlForRequest(apiBase, requestId) {
...
async function pollForCompletion(slug) {
  ...
  while (progressState && !progressState.cancelled) {
    ...
    pollIter++;
    const statusUrl = statusUrlForRequest(API_BASE, progressState?.requestId);
    if (pollIter % STATUS_CHECK_INTERVAL_POLLS === 0 && statusUrl) {
      try {
        const sr = await fetch(statusUrl, { credentials: "omit" });
        if (sr.ok) {
...
function startProgress(slug, themeLabel, requestId) {
...
async function submitTheme() {
  ...
  if (data?.ok && data.status === "queued") {
    ...
    const requestId = typeof data.request_id === "string" && REQUEST_ID_RE.test(data.request_id)
      ? data.request_id
      : null;
    if (slug && requestId) {
      startProgress(slug, raw, requestId);
...
async function init() {
  // Manifest, _quality.json badge rollup and the lineage quality read model
  // are independent, so fetch them in parallel — the gallery render needs
  // the first two but the manifest is the critical path and the quality
  // fetches are small, so we wait on all three before continuing rather than
  // streaming the gallery twice.
  const [manifest, quality, lineageQuality] = await Promise.all([
    loadManifest(),
    loadQuality(),
    loadLineageQuality(),
  ]);
  state.quality = quality;
  state.lineageQuality = lineageQuality;
  // Only strict quality-manifest rows that are ready+passed may enter the
  // picker/gallery or become a default selection.  The legacy manifest and
  // _quality.json are discovery/telemetry inputs, never publication gates.
  state.manifest = eligibleThemeManifest(manifest, lineageQuality);
  if (state.manifest.length === 0) {
    hideCanvasLoading();
    return;
  }
  ...
  state.data = await loadThemeArtifact(state.currentSlug);
  hideCanvasLoading();

  if (!state.data) {
    // A row can become stale between audit publication and artifact fetch.
    // Keep the whole interactive surface closed rather than exposing a shelf
    // whose selected item cannot be verified.
    return;
  }
  ...
  // Permalink focus resolution is canonical-only: the ?node= value is
  // matched against the strict artifact via canonical seed_paper_id, exact
  // strong alias, then graph-local ID — never a "first focus" fallback.
  // Lowercase 40-hex values reserve the canonical namespace and therefore do
  // not fall through to Semantic Scholar-shaped graph-local IDs on a miss.
  // Without ?node= the canonical root (highest-degree focus, graph-local ID
  // tie-break) is the landing target, which keeps the seed on-screen even on
  // wide trees where scrollLeft=0 used to show the sparse left edge.
  const requestedNode = new URLSearchParams(window.location.search).get("node");
  const landingFocus = LineageCore.resolveFocus(state.data, requestedNode);
  if (landingFocus) {
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if (requestedNode) focusNode(landingFocus.id);
      else scrollCanvasToNode(landingFocus.id);
    }));
  }
}
...
function focusNode(nodeId) {
  // CSS.escape: node ids come from the (LLM-derived) artifact and must not
  // be able to break out of the attribute selector.
  const fo = document.querySelector(`foreignObject[data-node-id="${CSS.escape(nodeId)}"]`);
  ...
  switch (mode) {
    ...
    case "novelty": {
      ...
      for (const e of edges) {
        ...
        else if (e.relation === "successor" || e.relation === "extends" || e.relation === "ablation") incremental += 1;
      }
...
function computeModeData(nodes, edges, mode) {
  ...
  if (mode === "novelty") {
    ...
    for (const e of edges) {
      if (incoming.has(e.dst)) incoming.get(e.dst).push(e.relation);
    }
    for (const n of nodes) {
      let disrupt = 0, incremental = 0;
...
function render() {
  ...
  const visibleEdges = (edges || []).filter((e) => state.visibleRelations.has(e.relation));
