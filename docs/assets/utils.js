...
(function (root) {
  ...
  PP.loadLineage = async function loadLineage(url = "lineage.json") {
    try {
      ...
      // job (conference) or the on-demand workflow (theme), and CF Pages
      // edge cache evicts on deploy. _headers max-age=300 + SWR=3600 is
      // appropriate. Polling paths that need stricter freshness pass
      // their own Request with explicit cache options upstream.
