...
catalogは既存 `/Users/taichi/.local/share/qwen-flash/cloud-flash-models.json` を利用。
...
呼出先は`/Users/taichi/.local/bin/qwen-implementation-queue [--interactive | --cloud-only] codex exec ...`。
入力argvは固定Flashモデル・loopback provider・`/Users/taichi/.codex-local-flash/models.json`を指定する。
Cloud選択時のみqueueが固定model/providerと`/Users/taichi/.local/share/qwen-flash/cloud-models.json`へ変換する。
...
具体的な共通routingの正本は設定担当の`/Users/taichi/.local/share/qwen-flash/HYBRID_ROUTING.md`である。
