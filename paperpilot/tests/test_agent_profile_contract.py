...
def test_retained_inactive_launcher_keeps_its_original_safety_contract() -> None:
    ...
    assert 'CATALOG="/Users/taichi/.codex-local-flash/models.json"' in launcher
    ...
    assert 'QUEUE = Path("/Users/taichi/.local/bin/qwen-implementation-queue")' in runner
    ...
    for term in (
        ...
        "--interactive",
        "--cloud-only",
        "/Users/taichi/.local/bin/qwen-implementation-queue",
    ):
