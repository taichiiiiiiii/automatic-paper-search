...
def evaluate(slug: str) -> dict:
    ...
    if not p.exists(): return {"error": "lineage.json missing"}
    d = json.loads(p.read_text())
    nodes = d.get('nodes', []); edges = d.get('edges', [])
    meta = d.get('meta', {}); theme = meta.get('theme', '')
    ...
    title_ref = sum(1 for e in edges if any(tok in (e.get('rationale','') or '').lower() for tok in title_tokens))
    title_ratio = title_ref / len(edges) if edges else 0
    rel_counts = Counter(e.get('rel','') for e in edges)
    total = sum(rel_counts.values())
    entropy = -sum((c/total)*math.log2(c/total) for c in rel_counts.values() if c > 0) if total else 0
    year_rev = sum(1 for e in edges
                   if (by_id.get(e.get('src','')) or {}).get('year') is not None
                   and (by_id.get(e.get('dst','')) or {}).get('year') is not None
                   and (by_id[e.get('src','')].get('year') or 0) > (by_id[e.get('dst','')].get('year') or 0) + 1)
    ...
    return {
        'theme': theme, 'slug': slug, 'nodes': len(nodes), 'focus': len(focus),
        'off_topic_focus': off_topic, 'edges': len(edges), 'template_count': tc,
        'template_ratio': tpl_ratio, 'title_ref_ratio': title_ratio,
...
def grade(r: dict) -> str:
    tpl, title = r.get('template_ratio', 1), r.get('title_ref_ratio', 0)
    base = 'D'
    if tpl <= 0.10 and title >= 0.50: base = 'A'
    elif tpl <= 0.30 and title >= 0.25: base = 'B'
    elif tpl <= 0.70 and title >= 0.10: base = 'C'
    # year reversal demote
