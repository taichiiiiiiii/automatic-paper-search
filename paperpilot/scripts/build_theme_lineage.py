...
from paperpilot.scripts._lineage_classify import (  # noqa: E402,F401
    # Re-exported for backwards compatibility — tests and external
    # callers still read these names off the script module. See the
    # _lineage_classify module docstring for the contract.
...
# ---- Paths & directories ----
DOCS_ROOT = ROOT / "docs"
# _CLASSIFICATION_CACHE_PATH was hoisted into paperpilot/scripts/
# _lineage_classify.py and is re-imported above for backwards compat.
...
# Heuristic classification constants (_INTENT_RELATION_MAP /
# _DEFAULT_DERIVED / _DERIVED_CONFIDENCE) live in _lineage_classify.py
# and are re-imported at module top for backwards compatibility.
...
def build_theme_lineage(
    *,
    ...
    # Pass the path explicitly so a test monkeypatch on
    # build_theme_lineage._CLASSIFICATION_CACHE_PATH is honoured even
    # though the helper lives in paperpilot.scripts._lineage_classify.
    provider, cache = _wrap_provider_with_cache(
        inner_provider, cache_path=_CLASSIFICATION_CACHE_PATH
    )
    logger.info(
