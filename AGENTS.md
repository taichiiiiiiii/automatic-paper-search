...
Keep commits small and independently reviewable or revertible. Split unrelated behavior, tests, configuration, generated records, and separate tasks when each is a valid standalone unit; do not bundle multiple completed tasks into one commit. Do not split an implementation from the directly required test, schema, or contract update when that would leave an invalid intermediate commit.

Use an issue-first workflow for every repository-scoped file change, experiment, durable research result, or external operation. Before work, the parent creates a GitHub Issue containing the goal, exact scope, acceptance checks, and publication/safety boundaries; Qwen and native subagents only report to the parent and never create, comment on, or close Issues. Use a non-protected branch named `codex/issue-<number>-<slug>` and reference the Issue in commits. After verification, the parent must push the branch, then add an Issue result comment with outcome, changed paths, checks, commit and branch, plus remaining risks. Close the Issue only after that push succeeds and acceptance criteria are met. If work, verification, commit, or push is incomplete, record the blocker and keep the Issue open. Emergency secret/publication-protection actions and simple conversational or read-only status answers are exempt; any follow-up repository work is not.
